...
$sql = "SELECT COUNT(*) FROM user_information WHERE cardstatus = 'approve' AND date_created >= NOW() - INTERVAL 1 DAY ";
if ($result = mysqli_query($conn, $sql)) {
  $row = $result->fetch_row();
  $total_user_card_approved = $row[0];
}

$sql = "SELECT COUNT(*) FROM user_information WHERE cardstatus = 'reject' AND date_created >= NOW() - INTERVAL 1 DAY";
if ($result = mysqli_query($conn, $sql)) {
  $row = $result->fetch_row();
  $total_user_card_reject = $row[0];
}

$sql = "SELECT COUNT(*) FROM user_information WHERE verificcode = '1' AND date_created >= NOW() - INTERVAL 1 DAY";
if ($result = mysqli_query($conn, $sql)) {
  $row = $result->fetch_row();
...
$sql = "SELECT  * FROM visitor GROUP   BY  ip_address";
if ($result = mysqli_query($conn, $sql)) {
  $total_visitor_count_new = mysqli_num_rows($result);
}
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="layout-demo-wrapper">
              ...
              <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <div class="row">
                  <div class="">
                    <h2><?php echo $total_visitor_count_new; ?></h2>
                  </div>
                  <?php if ($ssl_checker == '') { ?>
                    <!-- warning -->
                  ...
                  <div class="col-md-12 col-xl-12 ip-address-class">
                    <h3><?php
                  ...
                  <h3><?php
                      ...
                      // print_r($result);exit;
                      $login_count = $id = $result[0]['user_log_count'];
                      // Track the current user
                      $_SESSION['online'] = time();

                      // Set the timeout for considering a user offline (in seconds)
                      $timeout = 300; // 5 minutes

                      // Count the number of online users
                      $count = 0;
                      foreach ($_SESSION as $time) {
                        if (is_numeric($time) && time() - $time <= $timeout) {
                          $count++;
                        }
                      }

                      echo "在线用户数: " . $login_count;
                      ?></h3>
                      ...
                      <div class="d-flex align-items-end row">
                        ...
                        <div class="col-sm-5 text-center text-sm-left" style="text-align: right !important;">
                          <div class="card-body pb-0 px-0 px-md-0">
                            <img src="./assets/img/custom/man-with-laptop.png" height="140" alt="View Badge User" data-app-dark-img="custom/man-with-laptop.png" data-app-light-img="custom/man-with-laptop.png" />
                          </div>
                        </div>
                      </div>
                    </div>
